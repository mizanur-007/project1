...
$form_id= filter_input(INPUT_POST, 'd01');
$sql = "SELECT * FROM dog WHERE d_id='$form_id' ";
$result = mysqli_query($con, $sql);

if (mysqli_num_rows($result) > 0) {
    echo '<table style="width:80%; height:100px">';
  while($row = mysqli_fetch_assoc($result)) {
    echo "<tr><td>Dog ID</td><td>Dog Name</td><td>Dog Price</td><td>Dog Food</td></tr><tr><td>" . $row["d_id"]. "</td><td> " . $row["d_name"]. "</td><td>" . $row["d_price"]. "</td><td>" .$row["d_food"]."</td></tr><br>";
  }
  echo '</table>';
} else {
  echo "0 results";
